...
def evaluate(board, player, other_player):
    ...
    for i in range(3):
        player_score = sum(board[i][j] == player for j in range(3))
        opponent_score = sum(board[i][j] == other_player for j in range(3))
        score += heuristic[player_score][opponent_score]

    for j in range(3):
        player_score = sum(board[i][j] == player for i in range(3))
        opponent_score = sum(board[i][j] == other_player for i in range(3))
        score += heuristic[player_score][opponent_score]

    player_score = sum(board[i][i] == player for i in range(3))
    opponent_score = sum(board[i][i] == other_player for i in range(3))
    score += heuristic[player_score][opponent_score]

    player_score = sum(board[i][2 - i] == player for i in range(3))
    opponent_score = sum(board[i][2 - i] == player for i in range(3))
    score += heuristic[player_score][opponent_score]
